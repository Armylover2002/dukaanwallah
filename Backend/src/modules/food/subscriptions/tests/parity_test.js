...
async function runTest() {
    ...
    const MONGO_URI = 'mongodb+srv://powersafeindustries_db_user:GYNNDp6s5oDi8ILY@cluster0.mozt5cr.mongodb.net/itzofood?appName=Cluster0';
    ...
    try {
        ...
        for (const tc of testCases) {
            console.log(`\n[TEST] ${tc.label}`);
            try {
                await walletService.createTopupOrder(testPartnerId, userType, tc.amount);
                if (tc.expected === 'fail') {
        ...
        try {
            await walletService.createTopupOrder(testPartnerId, userType, 200);
            console.log("✅ PASSED: Order created successfully.");
        } catch (err) {
            console.error(`❌ FAILED: Expected success but got error: ${err.message}`);
        }
