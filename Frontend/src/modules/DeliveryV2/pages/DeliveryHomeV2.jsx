...
export default function DeliveryHomeV2({ tab = 'feed' }) {
  ...
  return (
    ...
    <div className={`relative w-full bg-white text-gray-900 ${currentTab === 'feed' ? 'h-screen overflow-hidden flex flex-col' : 'min-h-screen pb-[90px]'}`}>
      ...
      <div
        ...
        className={`relative w-full ${currentTab === 'feed' ? 'flex-1 overflow-y-auto pt-[120px]' : ''} no-scrollbar`}
      >
        {currentTab === 'feed' ? (
                  ...
                  <div className="absolute bottom-4 inset-x-0 z-[120] px-4">
                    {tripStatus === 'PICKED_UP' ? (
                      <div className="bg-white rounded-[3rem] p-8 shadow-[0_-20px_80px_rgba(0,0,0,0.4)] border border-gray-100 flex flex-col items-center">
                        ...
                        <ActionSlider label="Slide to Arrive" successLabel="Arrived ✓" disabled={!isWithinRange} onConfirm={reachDrop} color="bg-[#F26522]" />
                      </div>
                    ) : (
                      <button
      ...
      {!isKeyboardVisible && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 py-3 pb-[env(safe-area-inset-bottom,24px)] flex justify-between items-center z-[200] shadow-[0_-8px_20px_rgba(0,0,0,0.08)]">
