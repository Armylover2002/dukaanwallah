...
const Penalties = () => {
  ...
  useEffect(() => {
    ...
    // Fetch logic based on target type
    ...
    axiosInstance
      ...
      .then((res) => {
        ...
        if (targetType === 'restaurant') {
            items = res.data?.data?.restaurants || res.data?.restaurants || [];
        } else {
            items = res.data?.data?.deliveryPartners || res.data?.deliveryPartners || [];
        }
        
        setTargets(
          items.map((s) => ({
            id: s._id || s.id,
            name: targetType === 'restaurant' 
                ? (s.restaurantName || s.name || 'Unknown Restaurant')
                : (s.name || 'Unknown Delivery Partner'),
          }))
        );
      })
      .catch(() => toast.error(`Failed to load ${targetType}s`))
  ...
  const selectedTargetName = targets.find((s) => s.id === selectedTargetId)?.name || '';

  const handleApply = async () => {
    if (!selectedTargetId) { toast.error('Please select a target'); return; }
    ...
    setIsApplying(true);
    try {
      await axiosInstance.post('/food/admin/penalties/apply', {
        targetType,
    ...
    <div className="ds-section-spacing animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col gap-1 px-1 mb-6">
        ...
        <p className="ds-description">
          Apply a financial penalty to a restaurant or delivery partner. The amount will be deducted from their earnings.
        </p>
      </div>
      ...
      <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4">
        ...
        <p className="text-xs font-semibold text-amber-800 leading-relaxed">
          This action is <strong>irreversible</strong>. The penalty amount will be permanently deducted from the target's available balance and will appear as a negative adjustment in their transaction history.
        </p>
      </div>

      {/* Main Penalty Form */}
      <Card className="p-6 border-none shadow-sm ring-1 ring-red-100 bg-gradient-to-br from-red-50/40 to-white rounded-2xl mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="flex flex-col gap-2">
            ...
            <select
              ...
              className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-red-400/30 focus:border-red-400 transition-all mb-2"
            >
              <option value="restaurant">Restaurant</option>
              <option value="deliveryPartner">Delivery Partner</option>
            </select>

            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">
              Select {targetType === 'restaurant' ? 'Restaurant' : 'Delivery Partner'} <span className="text-red-500">*</span>
            </label>
            ...
            {selectedTargetId && (
              <p className="text-[10px] font-bold text-green-600 flex items-center gap-1 mt-1">
                <CheckCircle2 className="h-3 w-3" /> Selected: {selectedTargetName}
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4 md:col-span-2">
            <div className="flex flex-col gap-2">
              ...
              {penaltyAmount && Number(penaltyAmount) > 0 && (
                <p className="text-[11px] font-semibold text-red-600">
                  ₹{Number(penaltyAmount).toLocaleString('en-IN')} will be deducted from {targetType === 'restaurant' ? 'restaurant' : 'delivery partner'}'s earnings
                </p>
              )}
            </div>
            ...
            <div className="flex justify-end mt-2">
              <button
                onClick={handleApply}
                disabled={isApplying || !selectedTargetId || !penaltyAmount || !penaltyReason.trim()}
      ...
      <AnimatePresence>
        {history.length > 0 && (
          <motion.div
            ...
            className="mt-2"
          >
            <Card className="p-6 border-none shadow-sm ring-1 ring-slate-100 bg-white rounded-2xl mt-6">
              ...
              <div className="space-y-3">
                {history.map((item) => (
                  ...
                  >
                    <div>
                      <p className="text-xs font-bold text-slate-800">{item.target} <span className="text-[10px] text-slate-400 font-normal ml-2">({item.type === 'restaurant' ? 'Restaurant' : 'Delivery'})</span></p>
