...
const STATUS_CONFIG_TEMPLATE = {
  ...
  confirmed: { title: "Order Confirmed", color: "bg-green-600", iconType: 'food' },
  preparing: { title: null /* dynamic */, color: "bg-green-600", iconType: 'food' },
  assigned: { title: "Rider is arriving", color: "bg-green-600", iconType: 'rider' },
  at_pickup: { title: null /* dynamic */, color: "bg-green-600", iconType: 'rider' },
  ready: { title: "Handover in progress", color: "bg-green-600", iconType: 'rider' },
  on_way: { title: "Out for delivery", color: "bg-green-600", iconType: 'rider' },
  at_drop: { title: "Arrived at location", subtitle: "Please come to the door", color: "bg-green-600", iconType: 'rider' },
  delivered: { title: "Order delivered", color: "bg-green-600", iconType: 'delivered' },
...
export default function OrderTracking() {
  ...
  const handleCancelOrder = useCallback(() => {
    ...
    setRefundDestination(order?.payment?.refund?.requestedMethod === "wallet" ? "wallet" : "gateway");
    setShowCancelDialog(true);
  }, [order, isAdminAccepted, isEditWindowOpen]);
  ...
  const handleConfirmCancel = useCallback(async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        toast.success(response.data?.message || (paymentMethod === 'cash' || paymentMethod === 'cod'
        ...
        if (orderResponse.data?.success && orderResponse.data.data?.order)
          setOrder((prev) => transformOrderForTracking(orderResponse.data.data.order, prev));
      } else {
  ...
  }, [cancellationReason, orderId, isQuickOrder, isOnlinePaidForRefundChoice, refundDestination, order?.payment?.method, order?.paymentMethod]);
  ...
  const handleSubmitRating = useCallback(async () => {
    ...
    try {
      ...
      if (updatedOrderData) setOrder((prev) => transformOrderForTracking(updatedOrderData, prev));
        ...
        ...prev,
        ratings: { ...prev.ratings, restaurant: { rating: selectedRestaurantRating, comment: restaurantFeedbackText }, deliveryPartner: hasDeliveryPartner ? { rating: selectedDeliveryRating, comment: deliveryFeedbackText } : undefined },
      }));
        ...
        {!['at_pickup', 'ready', 'on_way', 'at_drop', 'delivered'].includes(orderStatus) && (
          <div className="px-4 pb-4 text-center">
            ...
            <motion.div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-full px-4 py-2" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }}>
              ...
              {orderStatus === 'preparing' && (<><span className="w-1 h-1 rounded-full bg-white" /><span className="text-sm text-orange-200">On time</span></>)}
              ...
              <div className="mt-5 rounded-2xl border border-white/70 bg-white/90 p-4">
                <div className="flex items-start gap-3">
                  <div className="flex flex-col items-center pt-1">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-red-100 text-[#FE5502]"><MapPin className="h-5 w-5" /></div>
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-4 mt-2">
                ...
                <div>
                  ...
                  <span className="text-sm font-bold text-green-600 uppercase">{order?.status?.replace('_', ' ')}</span>
                </div>
              </div>
            </div>
            {order?.note && (
      ...
      <AnimatePresence>
        {ratingModal.open && ratingModal.order && (
          <div className="fixed inset-0 z-[210] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="w-full max-w-md rounded-3xl bg-white shadow-2xl overflow-hidden">
              <div className="bg-gradient-to-r from-green-600 to-emerald-600 px-6 py-5">
                <div className="flex items-center justify-between mb-1">
                  <h2 className="text-xl font-bold text-white flex items-center gap-2"><Star className="w-5 h-5 fill-white" />Rate Your Experience</h2>
                  <button type="button" onClick={handleCloseRating} className="text-white/80 hover:text-white transition-colors p-1 rounded-full hover:bg-white/20"><X className="w-5 h-5" /></button>
                </div>
              ...
              <div className="px-6 py-6 space-y-6">
                {[
                  ...
                  <div key={section.key} className={sIdx > 0 ? "pt-4 border-t border-gray-100" : ""}>
                    <p className="text-sm font-semibold text-gray-900 mb-3">{section.label} (out of 5)</p>
                    <div className="flex items-center justify-center gap-2 mb-3">
                      {[1, 2, 3, 4, 5].map((num) => (
                        <button key={`${section.key}-${num}`} type="button" onClick={() => section.setRating(num)} className="p-2 transition-transform hover:scale-125 active:scale-95">
                          <Star className={`w-10 h-10 transition-all ${(section.rating || 0) >= num ? "text-yellow-400 fill-yellow-400 drop-shadow-lg" : "text-gray-200 hover:text-yellow-200"}`} />
                        </button>
                      ))}
                    </div>
                    <Textarea rows={2} value={section.text} onChange={(e) => section.setText(e.target.value)} className="w-full rounded-xl border-2 border-gray-100 px-4 py-2 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 resize-none transition-all" placeholder={section.placeholder} />
                  </div>
                ))}
                <Button type="button" disabled={submittingRating || selectedRestaurantRating === null || (!!order?.deliveryPartnerId && selectedDeliveryRating === null)} onClick={handleSubmitRating} className="w-full rounded-xl bg-gradient-to-r from-green-600 to-emerald-600 text-white text-base font-bold h-12 hover:from-green-700 hover:to-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-green-500/20 flex items-center justify-center gap-2">
                  {submittingRating ? <><Loader2 className="w-5 h-5 animate-spin" />Submitting...</> : <><Check className="w-5 h-5" />Submit Ratings</>}
                </Button>
                {selectedRestaurantRating === null && <p className="text-[10px] text-center text-gray-400">Please select a rating to enable submission</p>}
              </div>
            </motion.div>
          </div>
        )}
